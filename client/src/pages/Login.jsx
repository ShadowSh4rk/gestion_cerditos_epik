...
const Login = () => {
    ...
    {/* State variables:  name, setter, and useState for initialization */}
    {/* e.g:  onChange= {event=> setName(event.target.value)} */}
      ...
      try {
        ...
        if(state === 'Sign Up'){
          ...
          if(data.success){
            setIsLoggedin(true)
            getUserData()
            console.log("USERDATA LLEGA", data)
            navigate('/')
          }else{
            toast.error(data.message)
          }
    ...
    <div className='flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br from-blue-200 to-purple-400'>
      ...
      <div className='bg-slate-900 p-10 rounded-lg shadow-lg w-full sm:w-96 text-indigo-200 text-sm'>
        ...
        <form onSubmit={onSubmitHandler}>
          {/* Name input, only displayed when state equals 'Sign Up' */}
          {state === 'Sign Up' && (
          ...
          {/* Password input, always displayed */}
          <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>

...
const ResetPassword = () => {
  ...
  const inputRefs = React.useRef([])
    ...
    //to go backwards with backspace key
    ...
    //to be aple to paste the code with ctrl+v
    const handlePaste = (e)=>{
      //get from the clipboard
      const paste = e.clipboardData.getData('text')
      //split into 6 digits and paste them
  ...
  return (
    <div className='flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-200 to-purple-400'>
      ...
      <form onSubmit={onSubmitEmail} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
        <h1 className='text-white text-2x1 font-semibold text-center mb-4'>Reset password OTP</h1>
        ...
        <button className='w-full py-2.5 bg-gradient-to-r from-indigo-500 to-indigo-900 text-white rounded-full mt-5'>Submit</button>
      </form>
      }
      ...
      <form onSubmit={onSubmitNewPassword} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
        <h1 className='text-white text-2x1 font-semibold text-center mb-4'>New password</h1>
